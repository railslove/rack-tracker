...
RSpec.describe Rack::Tracker::GoogleAnalytics do
  ...
  describe 'with events' do
    ...
    describe 'default' do
      ...
      it 'will show events' do
        expect(subject).to match(/ga\(\"send\",{\"hitType\":\"event\",\"eventCategory\":\"Users\",\"eventAction\":\"Login\",\"eventLabel\":\"Standard\"}\)/)
      end
    end
    ...
    describe 'with a event value' do
      ...
      it 'will show events with values' do
        expect(subject).to match(/ga\(\"send\",{\"hitType\":\"event\",\"eventCategory\":\"Users\",\"eventAction\":\"Login\",\"eventLabel\":\"Standard\",\"eventValue\":\"5\"}\)/)
      end
    end
  end
  ...
  describe 'with ecommerce events' do
    describe 'default' do
      ...
      it 'will add transaction' do
        ...
        expect(subject).to match(/ga\(\"ecommerce:addTransaction\",#{attributes}\);/)
      end

      it 'will submit cart' do
        expect(subject).to match(/ga\('ecommerce:send'\);/)
      end
    end
  end
  ...
  describe 'with enhanced ecommerce events' do
    describe 'default' do
      ...
      it 'will add product' do
        ...
        expect(subject).to match(/ga\(\"ec:addProduct\",#{attributes}\);/)
      end

      it 'will add action' do
        expect(subject).to match(/ga\(\"ec:setAction\",\"purchase\"\);/)
      end
    end
  end
  ...
  describe 'with parameters events' do
    ...
    it 'will render dimension parameter' do
      expect(subject).to match(/ga\('set', 'dimension1', 'pink'/)
    end
  end
  ...
  describe 'with custom domain' do
    ...
    it 'will show asynchronous tracker with cookieDomain' do
      expect(subject).to match(/ga\('create', 'somebody', {\"cookieDomain\":\"railslabs.com\"}\)/)
      expect(subject).to match(/ga\('send', 'pageview', window\.location\.pathname \+ window\.location\.search\)/)
    end
  end

  describe 'with user_id tracking' do
    subject { described_class.new(env, tracker: 'somebody', user_id: ->(env) { return env[:user_id] }).render }

    it 'will show asynchronous tracker with userId' do
      expect(subject).to match(/ga\('create', 'somebody', {\"userId\":\"123\"}\)/)
      expect(subject).to match(/ga\('send', 'pageview', window\.location\.pathname \+ window\.location\.search\)/)
    end
  end
  ...
  describe 'with enhanced_link_attribution' do
    ...
    it 'will embedded the linkid plugin script' do
      expect(subject).to match(/linkid.js/)
    end
  end
  ...
  describe 'with advertising' do
    ...
    it 'will require displayfeatures' do
      expect(subject).to match(/ga\('require', 'displayfeatures'\)/)
    end
  end
  ...
  describe 'with enhanced ecommerce' do
    ...
    it 'will require the enhanced ecommerce plugin' do
      expect(subject).to match(/ga\('require', 'ec'\)/)
    end
  end
  ...
  describe 'with ecommerce' do
    ...
    it 'will require the ecommerce plugin' do
      expect(subject).to match(/ga\('require', 'ecommerce', 'ecommerce\.js'\)/)
    end
  end
  ...
  describe 'with optimize' do
    ...
    it 'will require the optimize plugin with container ID' do
      expect(subject).to match(/ga\('require', 'GTM-1234'\)/)
    end
  end
  ...
  describe 'with anonymizeIp' do
    ...
    it 'will set anonymizeIp to true' do
      expect(subject).to match(/ga\('set', 'anonymizeIp', true\)/)
    end
  end

  describe 'with dynamic tracker' do
    subject { described_class.new(env, { tracker: ->(env) { return env[:misc] } }).render }

    it 'will call tracker lambdas to obtain tracking codes' do
      expect(subject).to match(/ga\('create', 'foobar', {}\)/)
    end
  end
  ...
  describe 'adjusted bounce rate' do
    ...
    it 'will add timeouts to push read events' do
      expect(subject).to include "setTimeout(function() { ga('send', 'event', '15_seconds', 'read'); },15000)"
      expect(subject).to include "setTimeout(function() { ga('send', 'event', '30_seconds', 'read'); },30000)"
    end
  end
  ...
  describe '#pageview_url_script' do
    ...
    context 'with explicit_pageview disabled' do
      ...
      it 'does not send a pageview event' do
        expect(subject).not_to include "ga('send', 'pageview',"
      end
    end
